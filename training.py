...
for document in documents:
    ...
    if len(bag) != len(words) or len(output_row) != len(classes):
        print(f"Inconsistency found in document: {document}")
        print(f"Length of bag: {len(bag)}, Expected: {len(words)}")
        print(f"Length of output_row: {len(output_row)}, Expected: {len(classes)}")
